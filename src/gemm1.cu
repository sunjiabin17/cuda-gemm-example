...
void launch_gemm_v1(size_t m, size_t n, size_t k, const T* alpha, const T *A,
  ...
  gemm_v1<T><<<grid_dim, block_dim, 0U, stream>>>(m, n, k, *alpha, A, lda, B, ldb,
                                                 *beta, C, ldc);
  // printf("C[0] = %f\n", C[0]);
}

template void launch_gemm_v1<float>(size_t m, size_t n, size_t k, const float* alpha,
                                    const float *A, size_t lda, const float *B,
                                    size_t ldb, const float* beta, float *C, size_t ldc,
                                    cudaStream_t stream);

template void launch_gemm_v1<double>(size_t m, size_t n, size_t k, const double* alpha,
